...
%www.pudn.com > Saleh-Valenzuela-channel-model.rar > SV_model_ct.m, change:2010-08-18,size:2890b
 


function [h,t,t0,np]=SVUPGR_model(Lam,lam,Gam,gam0,a,num_ch,b002,sdi,nlos) 
% S-V channel model 
% Input  
...
if nargin<9, nlos=0;  end     % LOS environment 
if nargin<8, sdi=0; end % 0dB 
if nargin<7, b002=1; end  %  power of first ray of first cluster 
...
for k=1:num_ch             % loop over number of channels 
   ...
   while (Tc<10*Gam)  % cluster loop     
     ...
     while (Tr<10*gam) % ray loop 
       ...
       r = sqrt(randn^2+randn^2)*sqrt(brm2/2);   % rayleigh distributed mean power pow_bkl molich wireless comm eq 5.17
